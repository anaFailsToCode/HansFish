...
def fisherShuffle():
            ...
            row = list("--------")
            index = random.randint(0,3)*2
            row[index] = 'b'
            index = random.randint(0,3)*2 + 1
            row[index] = 'b'
            pieces = 6
            while pieces == 6:
            ...
            return( setUp + '/pppppppp/8/8/8/8/PPPPPPPP/' + setUp.upper())
...
def main():
    ...
    hordeFen = 'rnbqkbnr/pppppppp/8/8/PPPPPPPP/PPPPPPPP/PPPPPPPP/PPPPKPPP'
    game_state = ChessEngine.GameState(fen)
    currentMoves = game_state.getValidMoves()
  
    whitePlayer = True # shows wether white is a human
    ...
    while game:
        ...
        for e in pygame.event.get(): #checks too see if there is any events
            if e.type == pygame.QUIT: #if the user exsits the program close it down
                ...
                sys.exit()
                 ...
                 #only allow the user to interact if the game is still going on
        ...
        # Blit the input_box rect.
                    ...
                    if  1000 > location[0] > 910 and 600 > location[1] > 510:
                        ...
                        color = color_inactive
                        active = False
                        text = 'enter the name of your save file:'
                        done = False
                ...
                # Change the current color of the input box.
                                    color = color_active if active else color_inactive
                                if event.type == pygame.KEYDOWN:
                                    if  text == 'enter the name of your save file:':
                                        text =''
                                    if active:
        ...
        # Blit the input_box rect.
                    ...
                    if  1100 > location[0] > 1020 and 760 > location[1] > 690:
                        if whitePlayer == True:
                          whitePlayer = False
                        else:
                            whitePlayer = True
                        drawInfo(screen, whitePlayer, blackPlayer)
                    if  1000 > location[0] > 900 and 760 > location[1] > 690:
                        if blackPlayer == True:
                          blackPlayer = False
                        else:
                            blackPlayer = True
            ...
            # key handler
                ...
                if e.key == pygame.K_r:  # reset the game when 'r' is pressed
                    ...
                    moveUndone = True
                if e.key == pygame.K_f:
                    game_state = ChessEngine.GameState(fisherShuffle())
                    currentMoves = game_state.getValidMoves()
                    clickedSquare = ()
                    playerClicks = []
                    moved = False
                    ...
                    moveUndone = True
                if e.key == pygame.K_h:
                    game_state = ChessEngine.GameState(hordeFen)
                    currentMoves = game_state.getValidMoves()
                    clickedSquare = ()
                    playerClicks = []
                    moved = False
        ...
        if not gameOver and not human_turn and not moveUndone:
            if not AiThinking:
                ...
                move_finder_process = Process(target=ChessAI.findBestMove, args=(game_state, currentMoves, return_queue, AIdepth))
...
def drawGameState(screen, game_state, currentMoves, clickedSquare):
    ...
    drawAIWhiteButton(screen)
    drawCoordinates(screen)
    drawAIBlackButton(screen)
...
def drawSaveButton(screen):
    ...
    screen.blit(DepthImage, pygame.Rect(910, 510, 70, 70))
def drawAIWhiteButton(screen):
    DepthImage = pygame.transform.scale(pygame.image.load("images/AiWhite.jfif"), (70, 70))
    screen.blit(DepthImage, pygame.Rect(1000, 690, 70, 70))
def drawAIBlackButton(screen):
    DepthImage = pygame.transform.scale(pygame.image.load("images/AiBlack.jfif"), (70, 70))
    screen.blit(DepthImage, pygame.Rect(910, 690, 70, 70))

def drawInfo(screen, whitePlayer, blackPlayer):
    AI =  pygame.font.SysFont('Comic Sans MS', 30).render(("white: human" if whitePlayer else "white: AI"), True, pygame.Color('white'))
    screen.blit(AI, (820, 0))
    AI =  pygame.font.SysFont('Comic Sans MS', 30).render(("black: human" if blackPlayer else "black: AI"), True, pygame.Color('white'))
    screen.blit(AI, (820, 30))
    AI =  pygame.font.SysFont('Comic Sans MS', 30).render(("Depth: " + str(AIdepth)), True, pygame.Color('white'))
    screen.blit(AI, (820, 60))
...
def drawMoveLog(screen, game_state, font):
    ...
    for i in range(0, len(move_texts), moves_per_row):
        ...
        if ChessAI.countMaterial(game_state) > 0:
            string += '+'
        string += str(ChessAI.countMaterial(game_state))

        material =  pygame.font.SysFont('Comic Sans MS', 30).render(string, True, pygame.Color('white'))
        if ChessAI.countMaterial(game_state) != 0:
            screen.blit(material, (820, 90))
...
def drawCoordinates(screen):
    for i in range(8,0,-1):
        ...
        rank = pygame.font.SysFont('Comic Sans MS', 16).render(str(9-i), True, pygame.Color((234, 235, 200) if i % 2 == 0 else (119, 154, 88)))
        
        screen.blit(rank, (5, 100 * i - 95))
    for i in range(1,9,1):
        file = pygame.font.SysFont('Comic Sans MS', 16).render(chr(i+96), True, pygame.Color((234, 235, 200) if i % 2 != 0 else (119, 154, 88)))
        
        screen.blit(file, (100 * i - 10, 775))
...
def pieceMoveMove(move, screen, board, clock):
    ...
    for frame in range(frame_count + 1):
        ...
        end_square = pygame.Rect((move.end_file)  * sSize, move.end_rank * sSize, sSize, sSize)
        pygame.draw.rect(screen, color, end_square)
        # draw captured piece onto rectangle
        if move.piece_captured != '--':
            if move.is_enpassant_move:
                enpassant_rank = move.end_file + 1 if move.piece_captured[0] == 'p' else move.end_file - 1
